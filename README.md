<!--- ![Banner](https://github.com/Anni-Bamwenda/Anni-Bamwenda/blob/main/banner.gif) --->

<!---
### Languages

![Python](https://img.shields.io/badge/-Python-000?style=for-the-badge&logo=Python)
![SQL](https://img.shields.io/badge/-SQL-000?style=for-the-badge&logo=MySQL)
![C](https://img.shields.io/badge/-C-000?style=for-the-badge&logo=C)
![Java](https://img.shields.io/badge/-Java-000?style=for-the-badge&logo=Java&logoColor=007396)
--->
<h1 align="center" title="...and I'm happy to see you here :)">👋🏾 Hello! I’m Anni</h1>

I’m a Software Engineer focused on building ML-backed automation and infrastructure tooling.

My work focuses on:
- Production-quality Python systems
- ML pipelines with real-world constraints
- Automation, reliability, and CI/CD
---

### Featured Projects

| Project | Description | Focus |
|--------|-------------|--------|
| [Multimodal ADHD & Gender Prediction](https://github.com/Anni-Bamwenda/multimodal-adhd-sex-prediction) | End-to-end ML system for multi-output prediction of ADHD diagnosis and gender using socio-demographic and brain-imaging data | ML pipelines, evaluation tradeoffs |
| [Email Optimization Assistant](https://github.com/Anni-Bamwenda/Email-Optimization-Assistant) | Production-grade automation for Gmail extraction and daily summaries using LLM APIs | Automation, APIs, CI/CD |
| [Metastatic Cancer Prediction](https://github.com/Anni-Bamwenda/Metastatic-Cancer-Prediction) | Predictive ML pipeline for diagnosis duration using demographic and clinical data | Feature engineering, modeling |
<!---| ⚙️ MLOps Model Deployment ..Upcoming.. | Deploys trained ML model via FastAPI + Docker with CI/CD testing | FastAPI, Docker, GitHub Actions | --->

---

### Tech Toolbox
- **Languages:** Python, Shell, SQL  
- **ML & Data:** scikit-learn, NumPy, Pandas, feature engineering, model evaluation  
- **DevOps:** Jenkins, Ansible, GitHub Actions, CI/CD  
- **AI/ML:** OpenAI API, Google API, NLP, Random Forests, Unsupervised Learning
---

### Current Focus Areas  
- Scaling ML systems and API-driven workflows  
- Reliable automation and CI/CD for ML pipelines  
- Practical MLOps patterns for production environments
  
📫 [LinkedIn](https://www.linkedin.com/in/annibamwenda/)
